...
hook.Add( "PostRender", "RenderDupeIcon", function()
	...
	--
	-- This is gonna take some cunning to look awesomenot
	--
	local Size = Dupe.Maxs - Dupe.Mins
	local Radius = Size:Length() * 0.5
	local CamDist = Radius / math.sin(math.rad( FOV) / 2) -- Works out how far the camera has to be away based on radius + fovnot
	...
	-- Enable lighting again (or it will affect outside of this loopnot)
	...
	--
	-- And save itnot  (filename is automatic md5 in dupes/)
	--
	if (engine.WriteDupe( Dupe, jpegdata)) then

		-- Disable the save buttonnot not
		hook.Run("DupeSaveUnavailable")
		hook.Run("DupeSaved")

		MsgN("Savednot ")

		-- TODO: Open tab and show dupenot
