...
--
-- Add the name of the net message to the string table (or it won't be able to sendnot)
--
...
end, nil, "Save the current dupenot ", { FCVAR_DONTRECORD } )
...
if (CLIENT) then
	...
	net.Receive("ReceiveDupe", function( len, client)
			...
			if (not uncompressed) then
				MsgN("Received dupe - but couldn't decompressnot ?")
				return
			end
