...
function SWEP:DrawHUD()
	...
	for k, v in pairs(self:GetToolObject().Information) do
		...
		if (not v.icon2 and not v.name:StartWith( "use") and v.name:EndsWith( "use")) then v.icon2 = "gui/e.png" end

		self.Icons = self.Icons or {}
		if (v.icon and not self.Icons[ v.icon ]) then self.Icons[ v.icon ] = Material( v.icon) end
		if (v.icon2 and not self.Icons[ v.icon2 ]) then self.Icons[ v.icon2 ] = Material( v.icon2) end
