...
function CCSpawn(player, command, arguments)
	...
	if (not gamemode.Call( "PlayerSpawnObject", player, arguments[ 1 ], arguments[ 2 ])) then return end
...
local function InternalSpawnNPC(Player, Position, Normal, Class, Equipment, SpawnFlagsSaved, NoDropToFloor)
	...
	-- We don't want them spawning any entity they likenot
	if (not NPCData) then
		if (IsValid( Player)) then
			Player:SendLua("Derma_Message( \"Sorrynot  You can't spawn that NPCnot \")")
		end
		return
	end
...
local function GenericNPCDuplicator(ply, mdl, class, equipment, spawnflags, data)
	...
	if (not gamemode.Call( "PlayerSpawnNPC", ply, class, equipment)) then return end

	local normal = Vector(0, 0, 1)

	local NPCList = list.Get("NPC")
	local NPCData = NPCList[ class ]
	if (NPCData and NPCData.OnCeiling) then normal = Vector( 0, 0, -1) end

	local ent = InternalSpawnNPC(ply, data.Pos, normal, class, equipment, spawnflags, true)

	if (IsValid( ent)) then
		local pos = ent:GetPos() -- Hacknot  Prevnets the NPCs from falling through the floor

		duplicator.DoGeneric(ent, data)

		if (not NPCData.OnCeiling) then
			ent:SetPos(pos)
			ent:DropToFloor()
		end
...
local function CanPlayerSpawnSENT(player, EntityName)
	...
	-- You're not allowed to spawn this unless you're an adminnot
...
function CCGiveSWEP(player, command, arguments)
	...
	-- You're not allowed to spawn thisnot
	if (( not swep.Spawnable and not player:IsAdmin()) or ( swep.AdminOnly and not player:IsAdmin())) then
		return
	end
