...
local function GetAllFiles(tab, folder, extension, path)
	...
	if (not files) then
		MsgN("Warningnot  Ignoring '" .. folder .. "' because we cannot search in itnot ")
		return
	end
...
hook.Add( "DrawOverlay","sandbox_search_progress", function()
	if (not IsValid( g_SpawnMenu) or not IsValid( g_SpawnMenu.SearchPropPanel) or expectedCalls == 1 or totalCalls == expectedCalls) then return end
	...
	if (pnl.VBar and pnl.VBar.Enabled) then maxw = maxw - 20 end
