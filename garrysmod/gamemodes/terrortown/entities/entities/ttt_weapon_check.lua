...
local function VectorInside(vec, mins, maxs)
	return (vec.x > mins.x and vec.x < maxs.x and vec.y > mins.y and vec.y < maxs.y and vec.z > mins.z and vec.z < maxs.z)
end

-- We use stuff from weaponry.lua here, like weapon types
local function HasWeaponType(ply, weptype)
	for _, wep in pairs(ply:GetWeapons()) do
		if IsValid(wep) and WEPS.TypeForWeapon(wep:GetClass()) == weptype then return true end
	end
...
local function HasNamed(name)
	return function(ply) return ply:HasWeapon(name) end
end
	...
	for _, ply in pairs(player.GetAll()) do
		if IsValid(ply) and ply:IsTerror() then
			...
			if VectorInside(pos, mins, maxs) or VectorInside(center, mins, maxs) then
				if check(ply) then return 1 end
			end
		end
	end
