...
--[[---------------------------------------------------------
	...
	Desc: Called before all rendering
		Return true to NOT render this frame for some reason (dangernot)
-----------------------------------------------------------]]
...
function GM:CalcVehicleView(Vehicle, ply, view)
	...
	local tr = util.TraceHull( {
		...
		filter = function(e)
			...
			return not c:StartWith("prop_physics") andnot c:StartWith( "prop_dynamic") and not c:StartWith( "prop_ragdoll") and not e:IsVehicle() and not c:StartWith( "gmod_")
		end,
...
function GM:PreventScreenClicks(cmd)
	...
	-- that you don't fire guns when opening the properties menu. Hollanot
	--
