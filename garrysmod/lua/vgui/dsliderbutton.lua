...
function PANEL:GetDelta()
	return self.elementDeltaX, self.elementDeltaY
end
...
function PANEL:SetButton(Name, Type)
	...
	CrBut:SetParent( self ); CrBut:SetText( Name )
...
function PANEL:SetAction(Left, Right, Idx)
	...
	if( Left ) then
		CrBut.DoClick = function()
			...
			if( not pS ) then error( "["..CrBut:GetText().."]: "..sE ) end
		end
	else
