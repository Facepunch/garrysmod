...
function Color( r, g, b, a )
	...
	elseif ( isstring( r ) ) then
		if ( tonumber( r ) ) then
			r = tonumber( r )
		elseif ( HTMLColors[ r:lower() ] ) then
			r = HTMLColors[ r:lower() ] +
			( g != nil and g or 255 ) * 2^24
		elseif ( r:sub( 1, 1 ) == "#" ) then
			r = tonumber( "0x" .. ( r:len() == 4 and
			( r:sub( 2, 2 ) .. r:sub( 2, 2 ) ..
			r:sub( 3, 3 ) .. r:sub( 3, 3 ) ..
			r:sub( 4 ) .. r:sub( 4 ) ) or
			r:sub( 2 ) ) ) +
	...
	if ( not r ) then
		error( "Invalid declaration of Color()!" )
	end

	return setmetatable( {
		a = bit.band( bit.rshift( r, 24 ), 0xFF ),
		r = bit.band( bit.rshift( r, 16 ), 0xFF ),
		g = bit.band( bit.rshift( r, 8 ), 0xFF ),
		b = bit.band( r, 0xFF )
	}, COLOR )
...
function COLOR:ToInt()
	...
	return self.a * 2^24 + self.r * 2^16 + self.g * 2^8 + self.b
