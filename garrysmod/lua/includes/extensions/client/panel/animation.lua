...
function meta:AnimationThinkInternal()
	...
	if (not self.m_AnimList) then return end -- This can happen if we only have term
...
local function MoveThink(anim, panel, fraction)
	...
	if (not anim.StartPos) then anim.StartPos = Vector( panel.x, panel.y, 0) end
...
local function SizeThink(anim, panel, fraction)
	...
	if (not anim.StartSize) then local w, h = panel:GetSize() anim.StartSize = Vector( w, h, 0) end
...
function meta:SizeTo(w, h, length, delay, ease, callback)
	...
	if (not anim.SizeX) then  w = self:GetWide() end
	if (not anim.SizeY) then  h = self:GetTall() end
...
local function ColorThink(anim, panel, fraction)
	...
	if (not anim.StartColor) then anim.StartColor = panel:GetColor() end
...
function meta:ColorTo(col, length, delay, callback)
	...
	-- We can only use this on specific panel typesnot
...
local function AlphaThink(anim, panel, fraction)
	...
	if (not anim.StartAlpha) then anim.StartAlpha = panel:GetAlpha() end
...
local function LerpPositions(anim, panel)
	...
	if (not panel.TargetPos) then return end
