...
function util.IsValidPhysicsObject(ent, num)
	...
	if (not ent:IsWorld() and MoveType ~= MOVETYPE_VPHYSICS) then return false end
...
function util.LocalToWorld(ent, lpos, bone)
	...
	if (ent:EntIndex() == 0) then
		...
		if (ent:GetPhysicsObjectNum(bone) ~= nil and ent:GetPhysicsObjectNum(bone):IsValid()) then
...
function util.StringToType(str, typename)
	...
	MsgN("util.StringToType: unknown type \"", typename, "\"not ")
