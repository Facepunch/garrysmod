...
reading = {
	...
	string 		= function( )
		if( net.ReadBool( ) ) then -- compressed or not
			return util.Decompress( net.ReadData( net.ReadUInt( 15 ) ) )
		else
			return net.ReadData( net.ReadUInt( 15 ) )
		end
	end,
	Vector		= function( ) -- these are staying until someone fixes the precision
		--https://github.com/Facepunch/garrysmod-issues/issues/1947
		-- remove and replace with the next commented line when it's fixed:
...
writing = {
	...
	string = function( x )
		...
		if( compressed and #compressed < #x ) then
			...
			net.WriteBool( false ) -- not compressed
