...
if (CLIENT) then
	...
	local function CPanelUpdate(panel)
		...
		if (bIsDirty) then
			...
			-- It's a hack but hey welcome to GModnot
	...
	function SetupUI()
		...
		if (not UndoPanel) then return end
...
function Finish(NiceText)
	...
	if (not Current_Undo.Owner:IsValid()) then return end
...
local function CC_UndoNum(pl, command, args)
	...
	if (not PlayerUndo[ index ][ UndoNum ]) then return end
