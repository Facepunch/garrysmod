...
--
-- Game specific player modelsnot  (EP2, CSS, DOD)
...
function RegisterClass(name, table, base)
	...
	if (base) then
		...
		if (not Type[ name ]) then ErrorNoHalt( "RegisterClass - deriving "..name.." from unknown class "..base.."not \n") end
...
function SetPlayerClass(ply, classname)
	...
	if (not Type[ classname ]) then ErrorNoHalt( "SetPlayerClass - attempt to use unknown player class "..classname .. "not \n") end
...
local function LookupPlayerClass(ply)
	...
	if (method and method.Player == ply and method.ClassID == id and method.Func) then return method end
...
function RunClass(ply, funcname, ...)
	...
	if (not func) then ErrorNoHalt( "Function "..funcname.." not found on player classnot \n") return end
