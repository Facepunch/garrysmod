...
function Add( event_name, name, func )
	...
	if ( !isstring( name ) and !IsValid( name ) ) then ErrorNoHaltWithStack( "bad argument #2 to 'Add' (string expected, got " .. type( func ) .. ")" ) return end
...
function Remove( event_name, name )
	...
	if ( !isstring( name ) and !IsValid( name ) ) then ErrorNoHaltWithStack( "bad argument #2 to 'Remove' (string expected, got " .. type( func ) .. ")" ) return end
