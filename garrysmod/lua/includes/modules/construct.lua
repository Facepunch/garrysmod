...
if SERVER then
	...
	function SetPhysProp(Player, Entity, BoneID, Bone, Data)
		...
		if (not Bone) then
			...
			if (not Bone or not Bone:IsValid()) then
				Msg("SetPhysProp: Error applying attributes to invalid physics objectnot \n")
			return end
	...
	local function MagnetOff(pl, magnet)
		...
		if (not magnet or not magnet:IsValid()) then return false end
