...
timer.Simple( 0, function()
	...
	--
	-- AUTO GMA CLEANUP
	-- Clean up orphaned/unused GMA files on menu load to free disk space.
	-- Runs after a delay to avoid slowing down menu init.
	--
	timer.Simple( 10, function()

		-- menu_cleanupgmas is a built-in console command that removes
		-- GMA files that are no longer subscribed to on Workshop
		if ( ConCommand ) then
			RunConsoleCommand( "menu_cleanupgmas" )
			MsgN( "[Startup] Auto GMA cleanup complete" )
		end
