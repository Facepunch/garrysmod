...
function PANEL:OnActivate()
	...
	-- Addon mount tracking
	self.AddonMountCount = 0
	self.AddonMountTimes = {}
	self.AddonMountStart = SysTime()
	self.CurrentMountingAddon = nil
...
function PANEL:OnDeactivate()
	...
	-- Log addon mount times if any were tracked
	if ( self.AddonMountCount and self.AddonMountCount > 0 ) then
		local totalTime = SysTime() - ( self.AddonMountStart or SysTime() )
		MsgN( "[Load Times] " .. self.AddonMountCount .. " addons mounted in " .. string.format( "%.1f", totalTime ) .. "s" )

		-- Log slowest addons
		if ( self.AddonMountTimes ) then
			local sorted = {}
			for name, time in pairs( self.AddonMountTimes ) do
				table.insert( sorted, { name = name, time = time } )
			end
			table.sort( sorted, function( a, b ) return a.time > b.time end )

			local limit = math.min( 5, #sorted )
			if ( limit > 0 ) then
				MsgN( "[Load Times] Slowest addons:" )
				for i = 1, limit do
					MsgN( "  " .. string.format( "%6.1fms", sorted[ i ].time * 1000 ) .. "  " .. sorted[ i ].name )
				end
			end
		end
	end
...
function PANEL:StatusChanged( strStatus )
	...
	--
	-- ADDON MOUNT PROGRESS TRACKING
	-- Parse addon/workshop mount status and expose to loading screen.
	-- Tracks which addon is currently being mounted and how long each takes.
	--
	local mountAddon = string.match( strStatus, "Mounting%s+(.-)%.%.%." )
		or string.match( strStatus, "Loading%s+addon%s+(.-)%.%.%." )
		or string.match( strStatus, "Workshop%s+complete:%s+(.+)" )

	if ( mountAddon ) then

		-- Record timing for the previous addon
		if ( self.CurrentMountingAddon ) then
			local elapsed = SysTime() - ( self.LastMountStart or SysTime() )
			self.AddonMountTimes[ self.CurrentMountingAddon ] = elapsed
		end

		self.AddonMountCount = self.AddonMountCount + 1
		self.CurrentMountingAddon = mountAddon
		self.LastMountStart = SysTime()

		-- Send mount progress to loading screen JS
		local progressStr = "Mounting addon " .. self.AddonMountCount .. ": " .. mountAddon
		self:RunJavascript( "if ( window.SetStatusChanged ) SetStatusChanged( '" .. progressStr:JavascriptSafe() .. "' )" )

		-- Also expose as structured data if the loading screen supports it
		self:RunJavascript( "if ( window.AddonMountProgress ) AddonMountProgress( " .. self.AddonMountCount .. ", '" .. mountAddon:JavascriptSafe() .. "' )" )
